...
if 'source' in df.columns:
    st.markdown("### 📊 Requirements per Bron")

    source_col1, source_col2, source_col3, source_col4 = st.columns(4)

    source_counts = df['source'].value_counts()
    source_badges = {
        'rdf': ('📤', 'RDF Upload', COLORS['primary']),
        'manual': ('✍️', 'Manual Input', COLORS['success']),
        'bulk_import': ('📊', 'Bulk Import', COLORS['accent'])
    }

    for idx, (source, count) in enumerate(source_counts.items()):
        icon, label, color = source_badges.get(source, ('❓', source, COLORS['text']))
        avg_score_source = df[df['source'] == source]['quality_score'].mean()
        pct = (count / len(df)) * 100

        with [source_col1, source_col2, source_col3, source_col4][idx % 4]:
            st.markdown(
                f'<div style="background-color:{color}20; padding:16px; margin:8px; border-radius:8px; border-left:4px solid {color}">'
                f'<div style="font-size:32px; text-align:center;">{icon}</div>'
                f'<div style="font-weight:bold; text-align:center; margin-top:8px;">{label}</div>'
                f'<div style="font-size:24px; color:{color}; font-weight:bold; text-align:center;">{count}</div>'
                f'<div style="font-size:12px; color:#6B7280; text-align:center;">({pct:.1f}%)</div>'
                f'<div style="font-size:14px; color:{color}; text-align:center; margin-top:4px;">Avg: {avg_score_source:.2f}/8</div>'
                f'</div>',
                unsafe_allow_html=True
            )
