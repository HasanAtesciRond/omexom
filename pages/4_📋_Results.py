...
with filter_col3:
    # Source filter
    available_sources = df['source'].unique().tolist() if 'source' in df.columns else []
...
# Apply source filter first if applicable
if source_filter and 'source' in df.columns:
    df_for_filter = df[df['source'].isin(source_filter)]
else:
    df_for_filter = df
...
for idx, row in display_df.iterrows():
    ...
    with st.expander(f"{row['id']} | Score: {quality_score}/8", expanded=False):
        ...
        with col1:
            ...
            source_badge = source_badges.get(source, '❓ Unknown')
            st.markdown(f"**Bron:** {source_badge}")
